...
function preprocessPoText(input: string): string {
  ...
  if (text.length > 180_000) {
    ...
    const keywordLines = text
      .split(/\n+/)
      .filter((line) => /po\s*no|purchase order|s\.?no|sl\.?no|qty|quantity|uom|unit\s*price|amount|cgst|sgst|igst|total|gst|payment terms|delivery date|approved by|order handled by/i.test(line))
      .slice(0, 800)
      .join("\n");
    text = `${head}\n\n[KEY_LINES]\n${keywordLines}`;
  }
